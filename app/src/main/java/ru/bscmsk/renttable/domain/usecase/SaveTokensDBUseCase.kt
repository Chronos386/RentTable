...
class SaveTokensDBUseCase(private val dbTokensRepository: DBTokensRepository) {
    fun execute(context: Context, tokensDataModel: TokensDataModel) {
        dbTokensRepository.saveTokensDB(context=context, tokens = TokensModel(tokensDataModel.accessToken,tokensDataModel.refreshToken))
    }
}